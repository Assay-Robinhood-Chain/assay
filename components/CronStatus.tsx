...
import { INGESTION_INTERVAL_HOURS, SCORING_SWEEP_INTERVAL_HOURS } from "@/lib/constants";
...
function useCountdown(intervalHours: number, anchorMinutesAgo: number) {
  ...
  if (!now) return { lastRun: null as Date | null, nextRun: null as Date | null, remaining: "—" };
  ...
  return (
    <div className="card-hover rounded-xl border border-line bg-card p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="font-mono text-[11px] text-faint">{cadence}</span>
      </div>
      ...
      <div className="mt-3 flex items-baseline justify-between border-t border-line-soft pt-3">
        <span className="text-[11.5px] text-faint">Next run in</span>
