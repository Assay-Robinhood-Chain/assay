...
import { STALE_DATA_THRESHOLD_HOURS, MIN_SAMPLE_SIZE_FOR_CONFIDENCE } from "@/lib/constants";
...
export function ProvisionalNote({ sampleSize }: { sampleSize: number }) {
  return (
    <p className="rounded-lg border border-dashed border-faint/60 bg-panel px-3.5 py-2.5 text-[12.5px] leading-relaxed text-ink-soft">
      This score is <strong>provisional</strong>. With a tracked sample of{" "}
      <span className="font-mono">{sampleSize}</span> launches — below the{" "}
      <span className="font-mono">{MIN_SAMPLE_SIZE_FOR_CONFIDENCE}</span>-launch confidence
      threshold — the star rating is capped at 1 regardless of the raw composite score. This
  ...
  return (
    <div className="rounded-lg border border-line-soft bg-panel px-4 py-6 text-center">
      ...
      <p className="mt-1 text-[12.5px] text-muted">
        This launchpad was just onboarded and has no tracked launches yet. A score will appear
        once the initial backfill completes.
      </p>
    </div>
  );
}
