...
export default function LiveFeed({ launchpads }: { launchpads: Launchpad[] }) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      idRef.current += 1;
      setEvents((prev) => [makeEvent(idRef.current, lp), ...prev].slice(0, MAX_BUFFER));
  ...
  const filtered = filter === "ALL" ? events : events.filter((e) => e.lpSlug === filter);
    ...
    <div className="rounded-xl border border-line bg-card">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line px-4 py-2.5">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {launchpads.map((lp) => (
            <FilterChip key={lp.slug} active={filter === lp.slug} onClick={() => setFilter(lp.slug)}>
      ...
      <div className="h-[280px] overflow-y-auto px-4 py-3 font-mono text-[12px] leading-relaxed">
        {filtered.length === 0 ? (
          <div className="grid h-full place-items-center text-faint">No events in buffer.</div>
        ) : (
          filtered.map((e) => (
            <div key={e.id} className="border-b border-line-soft/60 py-1.5 last:border-0">
              <span className="text-faint">{e.time}</span>{" "}
              <span className={`font-medium ${TAG_CLASS[e.tag]}`}>[{e.tag}]</span>{" "}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-line px-4 py-2 text-[11px] text-faint">
        <span>
          Events in buffer: <span className="font-mono text-ink-soft">{filtered.length}</span>
        </span>
        <span>
          Status:{" "}
  ...
  return (
    <button
      ...
      className={`rounded-full border px-2.5 py-1 font-mono text-[11px] transition-colors ${
        active ? "border-cobalt bg-cobalt-soft text-cobalt" : "border-line text-muted hover:border-faint"
      }`}
    >
      {children}
    </button>
