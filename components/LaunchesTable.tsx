  ...
  const rows = useMemo(() => {
    ...
    return [...launches].sort((a, b) => {
      switch (sortKey) {
        ...
        default:
          return dir * (new Date(a.launchDate).getTime() - new Date(b.launchDate).getTime());
      }
    });
        ...
        <tbody>
          {rows.map((l) => (
            <tr
              ...
              role="link"
              tabIndex={0}
              onClick={() => router.push(`/launchpad/${launchpadSlug}/${l.tokenAddress}`)}
              onKeyDown={(e) => {
                if (e.key === "Enter") router.push(`/launchpad/${launchpadSlug}/${l.tokenAddress}`);
              }}
            ...
            >
              <td className="px-4 py-2.5">
                ...
                <div className="font-mono text-[11px] text-faint">{l.symbol}</div>
              </td>
              <td className="px-4 py-2.5 text-ink-soft">{formatDate(l.launchDate)}</td>
              <td className="px-4 py-2.5">
                <div className="flex flex-wrap gap-1.5">
                  {l.isConfirmedRugpull && <Flag tone="down">Confirmed rug</Flag>}
                  {l.isGraduated && !l.isConfirmedRugpull && <Flag tone="up">Graduated</Flag>}
                  {!l.isGraduated && !l.isConfirmedRugpull && <Flag tone="muted">Active</Flag>}
                  {l.washTradingFlag && <Flag tone="gold">Wash-trading flag</Flag>}
                </div>
              </td>
              <td className="px-4 py-2.5 font-mono text-ink-soft">
                {formatMultiple(l.peakMultiple)}
              </td>
              <td className="px-4 py-2.5 font-mono text-ink-soft">{formatUsd(l.liquidityUsd)}</td>
              <td className="px-4 py-2.5 font-mono text-ink-soft">{formatUsd(l.volume24hUsd)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function Flag({ tone, children }: { tone: "up" | "down" | "gold" | "muted"; children: React.ReactNode }) {
  const cls = {
  ...
  return (
    <span className={`rounded-full px-2 py-0.5 text-[10.5px] font-medium whitespace-nowrap ${cls}`}>
      {children}
    </span>
  );
}
